# Wizard

> Scholarly magic-users who cast spells of explosive power, subtle deception, and spectacular transformation through exhaustive study of magic's inner workings.

**Source:** PHB 2024

## Overview

| Attribute       | Value                          |
|-----------------|--------------------------------|
| Hit Die         | d6                             |
| Primary Ability | Intelligence                   |
| Saves           | Intelligence, Wisdom           |
| Armor           | None                           |
| Weapons         | Simple weapons                 |
| Tools           | None                           |
| Starting HP     | 6 + CON mod                    |
| HP per Level    | 4 (or 1d6) + CON mod          |
| Skills          | Choose 2: Arcana, History, Insight, Investigation, Medicine, Nature, Religion |
| Starting Equip  | (A) 2 Daggers, Arcane Focus (Quarterstaff), Robe, Spellbook, Scholar's Pack, 5 GP; or (B) 55 GP |

## Wizard Features Table

| Level | Prof. | Features | Cantrips | Prepared Spells | 1st | 2nd | 3rd | 4th | 5th | 6th | 7th | 8th | 9th |
|-------|-------|----------|----------|-----------------|-----|-----|-----|-----|-----|-----|-----|-----|-----|
| 1  | +2 | Spellcasting, Ritual Adept, Arcane Recovery | 3 | 4  | 2 | - | - | - | - | - | - | - | - |
| 2  | +2 | Scholar                         | 3 | 5  | 3 | - | - | - | - | - | - | - | - |
| 3  | +2 | Wizard Subclass                 | 3 | 6  | 4 | 2 | - | - | - | - | - | - | - |
| 4  | +2 | Ability Score Improvement       | 4 | 7  | 4 | 3 | - | - | - | - | - | - | - |
| 5  | +3 | Memorize Spell                  | 4 | 9  | 4 | 3 | 2 | - | - | - | - | - | - |
| 6  | +3 | Subclass Feature                | 4 | 10 | 4 | 3 | 3 | - | - | - | - | - | - |
| 7  | +3 | —                               | 4 | 11 | 4 | 3 | 3 | 1 | - | - | - | - | - |
| 8  | +3 | Ability Score Improvement       | 4 | 12 | 4 | 3 | 3 | 2 | - | - | - | - | - |
| 9  | +4 | —                               | 4 | 14 | 4 | 3 | 3 | 3 | 1 | - | - | - | - |
| 10 | +4 | Subclass Feature                | 5 | 15 | 4 | 3 | 3 | 3 | 2 | - | - | - | - |
| 11 | +4 | —                               | 5 | 16 | 4 | 3 | 3 | 3 | 2 | 1 | - | - | - |
| 12 | +4 | Ability Score Improvement       | 5 | 16 | 4 | 3 | 3 | 3 | 2 | 1 | - | - | - |
| 13 | +5 | —                               | 5 | 17 | 4 | 3 | 3 | 3 | 2 | 1 | 1 | - | - |
| 14 | +5 | Subclass Feature                | 5 | 18 | 4 | 3 | 3 | 3 | 2 | 1 | 1 | - | - |
| 15 | +5 | —                               | 5 | 19 | 4 | 3 | 3 | 3 | 2 | 1 | 1 | 1 | - |
| 16 | +5 | Ability Score Improvement       | 5 | 21 | 4 | 3 | 3 | 3 | 2 | 1 | 1 | 1 | - |
| 17 | +6 | —                               | 5 | 22 | 4 | 3 | 3 | 3 | 2 | 1 | 1 | 1 | 1 |
| 18 | +6 | Spell Mastery                   | 5 | 23 | 4 | 3 | 3 | 3 | 3 | 1 | 1 | 1 | 1 |
| 19 | +6 | Epic Boon                       | 5 | 24 | 4 | 3 | 3 | 3 | 3 | 2 | 1 | 1 | 1 |
| 20 | +6 | Signature Spells                | 5 | 25 | 4 | 3 | 3 | 3 | 3 | 2 | 2 | 1 | 1 |

## Class Features

### Level 1: Spellcasting

**Cantrips.** You know 3 Wizard cantrips of your choice. Whenever you finish a Long Rest, you can replace one cantrip with another Wizard cantrip. You learn additional cantrips at levels 4 and 10.

**Spellbook.** Your spellbook is a Tiny object (3 lbs, 100 pages) readable only by you or someone casting Identify. It starts with 6 level 1 Wizard spells of your choice. You add 2 Wizard spells to your spellbook each time you gain a Wizard level after 1.

- **Copying a Spell into the Book:** When you find a level 1+ Wizard spell, you can copy it if it's of a level you can prepare. For each spell level: 2 hours and 50 GP.
- **Copying the Book:** 1 hour and 10 GP per level of the copied spell.

**Spell Slots.** See the Wizard Features table. You regain all expended spell slots on a Long Rest.

**Prepared Spells.** You prepare spells from your spellbook. At level 1 you prepare 4 spells. The number increases per the table. You can change your prepared spells whenever you finish a Long Rest.

**Spellcasting Ability.** Intelligence.

**Spellcasting Focus.** Arcane Focus or your spellbook.

### Level 1: Ritual Adept

You can cast any spell as a Ritual if it has the Ritual tag and is in your spellbook. You needn't have the spell prepared, but you must read from the book.

### Level 1: Arcane Recovery

When you finish a Short Rest, you can recover expended spell slots with a combined level equal to no more than half your Wizard level (round up). None of the recovered slots can be level 6+. Once used, you can't use this feature again until you finish a Long Rest.

### Level 2: Scholar

Choose one skill you're proficient in from: Arcana, History, Investigation, Medicine, Nature, or Religion. You gain Expertise in that skill.

### Level 3: Wizard Subclass

You gain a Wizard subclass of your choice: Abjurer, Diviner, Evoker, or Illusionist. Subclass features at levels 3, 6, 10, and 14.

### Level 4: Ability Score Improvement

You gain the Ability Score Improvement feat or another feat of your choice. Repeats at levels 8, 12, and 16.

### Level 5: Memorize Spell

Whenever you finish a Short Rest, you can study your spellbook and replace one level 1+ prepared Wizard spell with another level 1+ spell from the book.

### Level 18: Spell Mastery

Choose a level 1 and a level 2 spell in your spellbook that have a casting time of an action. You always have those spells prepared and can cast them at their lowest level without expending a spell slot. To cast at a higher level, you must expend a spell slot. You can swap one of these spells on a Long Rest.

### Level 19: Epic Boon

You gain an Epic Boon feat or another feat of your choice. Boon of Spell Recall is recommended.

### Level 20: Signature Spells

Choose two level 3 spells in your spellbook as signature spells. You always have them prepared and can cast each once at level 3 without a spell slot. Recharges on a Short or Long Rest. To cast at a higher level, you must expend a spell slot.

## Subclasses (Level 3)

### Abjurer
> Specialists in protective magic—blocking, banishing, and shielding against harmful effects.

#### Level 3: Abjuration Savant
Choose two Wizard spells from the Abjuration school (each no higher than level 2) and add them to your spellbook for free. Whenever you gain access to a new level of spell slots, you can add one Abjuration Wizard spell to your spellbook for free.

#### Level 3: Arcane Ward
When you cast an Abjuration spell with a spell slot, you can create a magical ward on yourself (lasts until Long Rest). The ward has HP equal to **twice your Wizard level + your Intelligence modifier**. When you take damage, the ward takes it instead (apply Resistances/Vulnerabilities first). When reduced to 0 HP, you take remaining damage.

Recharging: When you cast an Abjuration spell with a spell slot, the ward regains HP equal to twice the spell slot level. Alternatively, as a Bonus Action, expend a spell slot to recharge it the same way.

Once you create the ward, you can't create it again until you finish a Long Rest.

#### Level 6: Projected Ward
When a creature you can see within 30 feet takes damage, you can use your Reaction to have your Arcane Ward absorb the damage. If the ward drops to 0 HP, the warded creature takes the remaining damage.

#### Level 10: Spell Breaker
You always have Counterspell and Dispel Magic prepared. You can cast Dispel Magic as a Bonus Action. You can add your Proficiency Bonus to ability checks made for Counterspell and Dispel Magic. If you fail the ability check for either spell, the spell slot is not expended.

#### Level 14: Spell Resistance
You have Advantage on saving throws against spells. You have Resistance to damage dealt by spells.

---

### Diviner
> Seers who peer into the future, unravel mysteries, and shape fate with their prophetic visions.

#### Level 3: Divination Savant
Choose two Wizard spells from the Divination school (each no higher than level 2) and add them to your spellbook for free. Whenever you gain access to a new level of spell slots, you can add one Divination Wizard spell for free.

#### Level 3: Portent
When you finish a Long Rest, roll two d20s and record the numbers (your portent dice). You can replace any Attack Roll, Saving Throw, or Ability Check made by you or a creature that you can see with one of these portent dice. You must choose to do so before the roll. Each portent die can be used only once. Unused portent dice are lost when you finish a Long Rest.

#### Level 6: Expert Divination
When you cast a Divination spell of level 2+ using a spell slot, you regain one expended spell slot. The regained slot must be of a level lower than the spell you cast and can't be higher than level 5.

#### Level 10: The Third Eye
As a Bonus Action, you can gain one of the following benefits, which lasts until you start a Long Rest or use this feature again:
- **Darkvision.** You gain Darkvision to 120 feet.
- **Greater Comprehension.** You can read any language.
- **See Invisibility.** You can see invisible creatures and objects within 10 feet of you that are within line of sight.

#### Level 14: Greater Portent
You roll three d20s for your Portent feature, instead of two.

---

### Evoker
> Masters of destructive energy who shape raw magical power into devastating—and precisely targeted—attacks.

#### Level 3: Evocation Savant
Choose two Wizard spells from the Evocation school (each no higher than level 2) and add them to your spellbook for free. Whenever you gain access to a new level of spell slots, you can add one Evocation Wizard spell for free.

#### Level 3: Sculpt Spells
When you cast an Evocation spell that forces other creatures you can see to make a saving throw, you can choose a number of those creatures equal to 1 + the spell's level. The chosen creatures automatically succeed on their saving throws against the spell and take no damage if they would normally take half damage on a successful save.

#### Level 6: Potent Cantrip
When a creature succeeds on a saving throw against one of your cantrips, the creature takes half the cantrip's damage (if any) but suffers no additional effect from the cantrip.

#### Level 10: Empowered Evocation
Whenever you cast a Wizard Evocation spell, you can add your Intelligence modifier to one damage roll of that spell.

#### Level 14: Overchannel
When you cast a Wizard spell of levels 1–5 that deals damage, you can deal maximum damage with that spell on the turn you cast it.

The first time you use this feature after a Long Rest, you suffer no adverse effects. If you use it again before finishing a Long Rest, you take 2d12 Necrotic damage per spell level immediately after casting. Each subsequent use before a Long Rest increases the Necrotic damage per spell level by 1d12.

---

### Illusionist
> Weavers of shadow and deception whose illusions blur the line between imagination and reality.

#### Level 3: Illusion Savant
Choose two Wizard spells from the Illusion school (each no higher than level 2) and add them to your spellbook for free. Whenever you gain access to a new level of spell slots, you can add one Illusion Wizard spell for free.

#### Level 3: Improved Illusions
You always have the Silent Image spell prepared. When you cast it, you can cast it as a Bonus Action. When you cast an Illusion spell that has a casting time of an action, you can change the casting time to a Bonus Action for that casting.

Additionally, you can make your Illusion spells seem more real. When a creature uses its action to examine one of your Illusion spells with an ability check, the DC equals your Spell Save DC.

#### Level 6: Phantasmal Creatures
You always have the Summon Beast spell prepared. When you cast it, you can change its school to Illusion, causing the beast to appear spectral. When cast this way, the spell doesn't require Concentration, but the spell lasts for a number of rounds equal to your Intelligence modifier (minimum 1 round).

#### Level 10: Illusory Self
When a creature makes an attack roll against you, you can use your Reaction to create an illusory duplicate. The attack automatically misses, and then the illusion dissipates. You can use this feature a number of times equal to your Proficiency Bonus, and you regain all expended uses on a Long Rest.

#### Level 14: Illusory Reality
When you cast an Illusion spell of level 1+, you can choose one inanimate, nonmagical object that is part of the illusion and make that object real. The object remains real for 1 minute, during which it can't deal damage or directly harm anyone. For example, you could make an illusory bridge real long enough for your allies to cross a chasm.

## 2024 Changes

Key differences from the 2014 Wizard:

- **Subclasses reduced:** 2024 PHB has 4 subclasses (Abjurer, Diviner, Evoker, Illusionist) instead of 8 school-based subclasses. The old Conjurer, Enchanter, Necromancer, and Transmuter subclasses are not in the 2024 PHB.
- **Subclass at Level 3:** Was level 2 in 2014; now level 3 to match most other classes.
- **Scholar (Level 2):** New feature granting Expertise in an INT-based skill. Replaces nothing specific (fills the gap from subclass moving to level 3).
- **Memorize Spell (Level 5):** New feature allowing prepared spell swaps on Short Rests.
- **Arcane Recovery:** Now specifically limited to no level 6+ spell slots (was "level 5 or lower" in some readings).
- **Cantrip swapping:** Can now replace a cantrip on Long Rests (was fixed in 2014).
- **Ritual Adept:** Renamed from Ritual Casting, functionally identical.
- **Spell Mastery (Level 18):** Can now swap chosen spells on a Long Rest.
- **Signature Spells (Level 20):** Recharge on Short Rest instead of Long Rest.
- **Epic Boon (Level 19):** Replaces the old Level 20 capstone timing; ASI at level 19 becomes Epic Boon.
- **Savant features reworked:** Each subclass Savant feature now gives free spells in the spellbook and one free spell per new spell level, rather than halving gold/time to copy.
- **Abjurer Ward:** Ward HP now = 2× Wizard level + INT mod (was just 2× Wizard level). Can recharge via Bonus Action spell slot expenditure.
- **Spell Breaker (Abjurer Level 10):** New—replaces the old Improved Abjuration. Failed checks don't expend the spell slot.
- **Diviner Portent:** Functionally similar but clarified.
- **Evoker Overchannel:** Increased self-damage penalty for repeated use (2d12 per spell level, +1d12 each subsequent use).
- **Illusionist reworked:** Improved Illusions, Phantasmal Creatures, and Illusory Self are significantly different from the 2014 versions.
