---
type: index
version: dnd-5e-2024
updated: 2026-05-19
---

# Party Index

## Current Party (Level 1)

| Character | Player | Species | Class | Background | Links |
|-----------|--------|---------|-------|------------|-------|
| moco | Carmen | [Wood Elf](../../species/elf.md) | [Druid](../../classes/druid.md) (Circle of the Land) | [Acolyte](../../backgrounds/acolyte.md) | [Character Sheet](moco.md) |
| Han | Han | [Tiefling](../../species/tiefling.md) | [Wizard](../../classes/wizard.md) (Divination) | [Sage](../../backgrounds/sage.md) | [Character Sheet](han.md) |
| Helen | Helen | [High Elf](../../species/elf.md) | [Wizard](../../classes/wizard.md) (Abjuration) | [Sage](../../backgrounds/sage.md) | [Character Sheet](helen.md) |
| xixi | xixi | [High Elf](../../species/elf.md) | [Paladin](../../classes/paladin.md) (Oath of Vengeance) | [Noble](../../backgrounds/noble.md) | [Character Sheet](xixi.md) |

## Key Feats & Spells by Character

### moco (Druid)
- **Origin Feat:** [Magic Initiate (Cleric)](../../feats/origin/magic-initiate.md) — Spellcasting ability: Wisdom
- **Key Spells:** [Produce Flame](../../spells/cantrips/produce-flame.md), [Starry Wisp](../../spells/cantrips/starry-wisp.md), [Speak with Animals](../../spells/1st-level/speak-with-animals.md), [Bless](../../spells/1st-level/bless.md)

### Han (Divination Wizard)
- **Origin Feat:** [Magic Initiate (Wizard)](../../feats/origin/magic-initiate.md)
- **Key Feature:** Portent (Divination subclass)

### Helen (Abjuration Wizard)
- **Origin Feat:** [Magic Initiate (Wizard)](../../feats/origin/magic-initiate.md)
- **Key Feature:** Arcane Ward (Abjuration subclass)

### xixi (Vengeance Paladin)
- **Origin Feat:** [Skilled](../../feats/origin/skilled.md)
- **Key Feature:** Vow of Enmity (Oath of Vengeance)
